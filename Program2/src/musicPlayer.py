...
class Player:
    ...
    def start_count(self, t):
        ...
        while current_time <= t and not self.music_stopped:
    ...
    def play_music(self):
        ...
        if not self.paused:
            ...
            try:
                ...
                if self.next:
                    if self.num == len(self.playlist)-1:
                        self.num = 0
                    else:
                        self.num += 1
                    play_it = self.playlist[self.num]
                    self.next = False
                elif self.prev:
                    if self.num == 0:
                ...
                tkinter.messagebox.showerror('File not found',
                                             'JMusic could not find the file. Please choose a music file to play')
        else:
            mixer.music.unpause()
            self.paused = False
    ...
    @staticmethod
    def about_us():
        tkinter.messagebox.showinfo('About JMusic', 'This is a music player built using python and tkinter')
...
root = Tk()
img = PhotoImage(file='musicplayer.ico')
...
current_timeLabel = Label(top_frame, text='current time : --:--')
...
# prev song
prevButton = Button(root, text='⏮', command=player.prev_music)
prevButton.place(x=300, y=170, width=50, height=20)

# next song
nextButton = Button(root, text='⏭', command=player.next_music)
nextButton.place(x=390, y=170, width=50, height=20)
...
statusBar = Label(root, text='Welcome to JMusic', relief=SUNKEN, anchor=W)
